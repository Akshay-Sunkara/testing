...
function PromptBar() {
  ...
  const [isLoadingSlackdump, setIsLoadingSlackdump] = useState(false)
  ...
  const handleTestSlackdump = async () => {
    setIsLoadingSlackdump(true)
    
    try {
      const response = await fetch(`${API_BASE_URL}/api/extract/slackdump`, {
        method: 'POST',
        headers: { 
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to test slackdump')
      }
      ...
      alert(`❌ Error: ${error.message}`)
      console.error('Slackdump test error:', error)
    } finally {
      setIsLoadingSlackdump(false)
    }
  }
    ...
    <div className="prompt-container">
      <div className="prompt-bar">
        ...
        <div className="button-group">
          ...
          <button 
            className={`action-btn slackdump-btn ${isLoadingSlackdump ? 'loading' : ''}`}
            onClick={handleTestSlackdump}
            disabled={isLoadingSlackdump}
            title="Test slackdump CLI tool (alternative to Slack Bot API)"
          >
            {isLoadingSlackdump ? (
              <>
                <span className="spinner"></span>
